...
> No description, just file

## Files

- [keygenme-trial.py](https://challenge-files.picoctf.net/c_wily_courier/c9ca91017c43622b68a394b99b485293c91e4e48a6040750b7c19f1098a84d71/keygenme-trial.py)
...
No description and no hints. Seems like everything needed is in the file. Will need to inspect it closely.

---

## Analysis

The program opens up to be a trial version of an Arcane Calculator. There are some global variables, the important ones here are:
- username_trial: a string
- bUsername_trial: a binary string
- key_part_static1_trial = "picoCTF{1n_7h3_kk3y_of_"
- key_part_dynamic1_trial = "xxxxxxxx"
- key_part_static2_trial = "}"
- key_full_template_trial = key_part_static1_trial + key_part_dynamic1_trial + key_part_static2_trial
The key is made up of 3 parts and is concatenated to unlock the full trial. Some methods that are present are:
- intro_trial
- menu_trial
- enter_license
- check_key

The last 2 being the most important.

The program starts with a greeting and asks the user to pick a choice a-d with safeguards to prevent picking a choice outside of this range. If option c is picked, the user is prompted with an input, expecting the flag. the flag is in 3 parts, two of which are in plaintext and can be put aside, the second part however, needs a closer examination.

`enter_license` asks for the key and strips it, removing the end of line character (EOL). It then calls `check_key` using the input and the `bUsername_trial` as arguments. It checks for each of the 8 hidden characters and compares them to `hashlib.sha256(username_trial).hexdigest()[a_number]`. This is the index of the[ hexdigest()](https://docs.python.org/3/library/hashlib.html#hashlib.hash.hexdigest) after the username was hashed with the [sha256()](https://www.simplilearn.com/tutorials/cyber-security-tutorial/sha-256-algorithm) algorithm.

By printing those out, I can get the redacted part of the key and use that in the program to get the flag. So after concatenating the hexdigest values into a new variable, and changing the variable name to use the binary username instead, I printed out the full flag/key.

The output is as follows:
```
What would you like to do, BENNETT (a/b/c/d)? c
Full Key is: picoCTF{1n_7h3_kk3y_of_08c46aa4}

Enter your license key: picoCTF{1n_7h3_kk3y_of_08c46aa4}
...
key_part_dynamic1_trial = "xxxxxxxx"
key_part_static2_trial = "}"
key_part_dynamic_unredacted = hashlib.sha256(bUsername_trial).hexdigest()[4]+hashlib.sha256(bUsername_trial).hexdigest()[5]+hashlib.sha256(bUsername_trial).hexdigest()[3]+hashlib.sha256(bUsername_trial).hexdigest()[6]+hashlib.sha256(bUsername_trial).hexdigest()[2]+hashlib.sha256(bUsername_trial).hexdigest()[7]+hashlib.sha256(bUsername_trial).hexdigest()[1]+hashlib.sha256(bUsername_trial).hexdigest()[8]
key_full_template_trial = key_part_static1_trial + key_part_dynamic_unredacted + key_part_static2_trial
...
(d) Exit Arcane Calculator")
    ...
    print(key_part_dynamic_unredacted)
...
def enter_license():
    print("Full Key is: " + key_full_template_trial)
...
- Python `hashlib` module (for SHA-256 hashing)
- Python `cryptography.fernet.Fernet` (used internally by the script to decrypt the full version blob)
- A Python-aware IDE or text editor for reading and lightly modifying the script
...
- Global variables computed at script load time (before `main()`/`ui_flow()` even runs) can sometimes hand you the answer directly.
- Obfuscation doesn't have to be cryptographically strong, simply reordering indices into a hash digest (instead of taking a contiguous slice) is enough to make a "just read the code" solution slightly harder without adding real security.
- This challenge reuses the Fernet + base64 decryption pattern from earlier challenges [[1-Python Wrangling]], showing how the same primitive (a password/key transformed into a Fernet-compatible key) recurs across different challenge themes.
---
...
- Keygen-style Reverse Engineering
- Hash-based Key Derivation
- Static Analysis of Python Source
- Fernet Symmetric Encryption
- Obfuscation via Non-sequential Indexing
